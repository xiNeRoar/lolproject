...
**C19: ladderSettings new fields — full stack (schema + OpenAPI + backend + codegen)**

IMPORTANT: The ManageLadderSettings UI already has these six fields rendered, but they are completely disconnected from the backend. The save payload uses a TypeScript force-cast that silently drops the unknown fields. Nothing is persisted. Must fix all four layers:

1. Schema — `lib/db/src/schema/ladderSettings.ts`: add columns:
   - maxDeclinesPerWeek: integer, default 2
   - maxDeclinesSameOpponentPerWeek: integer, default 1
   - noShowExpiryDays: integer, default 7
   - playoffMinPlayers: integer, default 4
   - playoffSize: integer, default 8
   - playoffFormat: text, default 'single_elimination'

2. OpenAPI — `lib/api-spec/openapi.yaml`: add all six fields to `LadderSettings` schema (both properties and required list)

3. Codegen — run `cd lib/api-spec && pnpm run generate` to regenerate types. Verify `LadderSettings` interface in `api.schemas.ts` includes all six fields.

4. Backend — `artifacts/api-server/src/routes/ladderSettings.ts`:
   - `formatSettings()`: add all six fields to the returned object
   - `PUT /` handler: read all six fields from req.body and include in the `.set({})` call
