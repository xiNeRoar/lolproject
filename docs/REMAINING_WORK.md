...
| F16 | **Season > Matches dialog — format reads from `ladderSettings.defaultMatchFormat`** | Replace hardcoded `format: "BO1"` in `onMatchSubmit` with value from the format dropdown. Dialog reads `GET /api/ladder-settings` for default; admin can override per-match via BO1/BO3/BO5 dropdown. Score placeholder updates dynamically (BO1 → "e.g. 1-0", BO3 → "e.g. 2-1", BO5 → "e.g. 3-2"). **BLOCKED on B10** (Claude must add `defaultMatchFormat` field to ladderSettings schema + OpenAPI first). | BLOCKED on B10 |
...
| B10 | **Add `defaultMatchFormat` to `ladderSettings`** | PRD Principle 2: "Never hardcode any configurable value." The current Season > Matches dialog hardcodes `format: "BO1"` in `onMatchSubmit` — a PRD violation. Fix: In `lib/db/src/schema/ladderSettings.ts`, add `defaultMatchFormat: text("default_match_format").notNull().default("BO1")`. Run `pnpm run db:push`. In `lib/api-spec/openapi.yaml`: add `defaultMatchFormat` (string, enum: ["BO1","BO3","BO5"]) to both the `LadderSettings` response schema and the `UpdateLadderSettingsRequest` body schema. In `artifacts/api-server/src/routes/ladderSettings.ts` (GET handler): include `defaultMatchFormat` in the SELECT/response. In the PUT handler: read `req.body.defaultMatchFormat` and pass to update. Run `pnpm run codegen` after OpenAPI change. This unblocks F16. | TODO |
| B11 | **Add `defaultMatchFormat` to `seasons`** | Match format inheritance hierarchy: `ladderSettings.defaultMatchFormat` → `seasons.defaultMatchFormat` (override) → `matches.format` (per-match override). Currently Seasons have no way to set a default match format — matches all fall back to hardcoded BO1. Fix: In `lib/db/src/schema/seasons.ts`, add `defaultMatchFormat: text("default_match_format")` (nullable, no default — null means inherit from ladderSettings). Run `pnpm run db:push`. In `lib/api-spec/openapi.yaml`: add `defaultMatchFormat` (string, nullable, enum: ["BO1","BO3","BO5"]) to the `Season` response schema and `CreateSeasonRequest`/`UpdateSeasonRequest` body schemas. In `artifacts/api-server/src/routes/seasons.ts` `formatSeason()`: include `defaultMatchFormat` in the response. In POST/PUT handlers: read `req.body.defaultMatchFormat` and persist. Run `pnpm run codegen`. Unblocks F19 (Season Details tab defaultMatchFormat field + match dialog inheritance). | TODO |
